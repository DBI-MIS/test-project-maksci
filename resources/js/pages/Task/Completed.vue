...
const pendingTasks = computed(() => {
    ...
    return Array.isArray(props.task)
        ? props.task.filter(t => t.status === 'completed')
        : [];
});
...
<template>
    <AppLayout>
        <div class="flex flex-col bg-gray-50">
            <div class="bg-blue flex justify-between bg-slate-200 h-20 p-5 items-center">
                ...
                <div class="flex gap-3">
                    <div class="bg-blue-300 w-30 h-10 rounded-2xl flex justify-center items-center">
                        <Link :href="TaskController.create()">
                            ADD Task
                        </Link>
                    </div>
                    <div class="bg-blue-300 w-30 h-10 rounded-2xl flex justify-center items-center">
                    ...
                    <div class="bg-blue-300 w-30 h-10 rounded-2xl flex justify-center items-center">
                        <Link :href="TaskController.index()">
                            Show Tasks
                        </Link>
                    </div>
                </div>
            ...
            <div class="p-3">
                <table class="min-w-full border justify-center text-center">
                    <thead>
                        <tr class="h-20 bg-mist-100">
                            <th>Task</th>
                            <th>Type</th>
                            <th>Due Date</th>
                            <th>Status</th>
                    ...
                    <tbody>
                        <tr v-for="tsk in pendingTasks" :key="tsk.id" class="h-15">
                            ...
                            <td>{{ tsk.due }}</td>
                            <td class="p-2">
                                <select :value="tsk.status" disabled @change="updateStatus(tsk.id, $event.target.value)" :class="[
